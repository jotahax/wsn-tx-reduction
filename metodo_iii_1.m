...
while(i<m)
    ...
    if abs(ma1 - pm10(i)) >= 1
        ...
        if q>1 %evitamos que q sea negativa
            q = q - 1; %disminuimos periodo
        end
    ...
    %caso 2: si son iguales lo aumentamos
     elseif abs(ma1 - pm10(i)) == 0 
        
        if q<10 %evitamos que q supere los 10 minutos de periodo
            q = q + 1; %aumentamos periodo
        end
        if (i+q)<=m %evitamos que la posicion supere la longitud del vector
            ma1 = pm10(i); %guardamos la muestra
            i = i + q; %obtenemos nueva posicion actual
